...
public class AuctionServiceImplTest {
	...
	public void testCreateAuction() throws InterruptedException {
		try {
			...
			Auction createdAuction = auctionService
					.getAuctionById(createdAuctionId);
	...
	public void testPlaceBidForAuction() throws Exception {
		...
		long auctionId = auctionService.createAuction(TEST_USER_ID, TEST_ITEM,
				TEST_END_TIME);
		Thread.sleep(100);
		...
		assertTrue("There should only be one bid", auctionAfterBid
				.getBidsPlaced().size() == 1);
		assertEquals("The bid placed should be same as expected", bid,
				auctionAfterBid.getBidsPlaced().iterator().next());
	}
	...
	public void testCleanupAuctions() throws InterruptedException, HgException {
		...
		Assert.assertTrue("The auction should be expired",
				TEST_END_TIME.before(currentDate));
		try {
			Auction auction = auctionService
					.getAuctionById(auctionToBeDeletedId);
	...
	public void testIsAuctionExpired() throws HgException {
		long auctionId = auctionService.createAuction(TEST_USER_ID, TEST_ITEM,
				TEST_END_TIME);
		assertTrue("The auction should be expired",
				auctionService.isAuctionExpired(auctionId));
	...
	public void testGetHighestBidForAuction() {
		try {
			long auctionId = auctionService.createAuction(TEST_USER_ID,
					TEST_ITEM, TEST_END_TIME);
			...
			Bid highestBidForAuction = auctionService
					.getHighestBidForAuction(auctionId);

			assertEquals("the highest bid for the auction should be 30",
					highestBid.getBidPrice(),
					highestBidForAuction.getBidPrice());
		} catch (HgException e) {
	...
	public void testGetAuctionsCreatedByUser() throws InterruptedException {
		...
		Collection<Auction> auctionCreatedByUser = auctionService
				.getAuctionsCreatedByUser(TEST_USER_ID);

		assertTrue("The collection should only have one element",
				auctionCreatedByUser.size() == 1);
		assertEqualAuctions(expectedAuction, auctionCreatedByUser.iterator()
				.next());
	...
	public void testGetAuctionsUserHasBidOn() throws InterruptedException {
		try {
			...
			User testUser = new User(TEST_USER_ID, UserType.BUYER, "", "", "", "");
			testUser.setBids(Arrays.asList(new Bid(95.0, TEST_ITEM, auctionId, TEST_USER_ID) ));
			...
			fail("The test should not fail at this point.");
		}
	...
	private void assertEqualAuctions(Auction expectedAuction, Auction auction) {
		Assert.assertEquals("The auction sellers should be equal",
				expectedAuction.getSellerId(), auction.getSellerId());
		Assert.assertEquals("The auction end times should be equal",
				expectedAuction.getEndTime(), auction.getEndTime());
		Assert.assertEquals("The auction items should be equal",
				expectedAuction.getAuctionItem(), auction.getAuctionItem());
		Assert.assertEquals("The auction bids should be equal",
				expectedAuction.getBidsPlaced(), auction.getBidsPlaced());
